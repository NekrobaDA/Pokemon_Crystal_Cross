...
; Johto Gym Leader TM gifts    ;+1
	const EVENT_GOT_TM31_MUD_SLAP      ;TM51_ROOST
	const EVENT_GOT_TM49_FURY_CUTTER   ;TM42_U_TURN
	...
	const EVENT_GOT_TM16_ICY_WIND      ;TM07_HAIL
	const EVENT_GOT_TM24_DRAGONBREATH  ;TM59_DRAGON_PULSE
; HMs                          ;+2
...
; HMs continued + special TMs  ;+3
	const EVENT_GOT_HM09_ROCK_CLIMB
	const EVENT_GOT_HM10_DIVE
	const EVENT_GOT_TM02_HEADBUTT
	const EVENT_GOT_TM10_HIDDEN_POWER
	const EVENT_GOT_TM47_STEEL_WING
	const EVENT_GOT_TM55_SHADOW_CLAW
	const EVENT_TEAM_ROCKET_BASE_B2F_TM_THIEF
	const EVENT_GOT_TM60_TERA_BURST
; Kanto Gym Leader TM gifts    ;+4
	const EVENT_GOT_TM_39_ROCK_SLIDE
	const EVENT_GOT_TM03_WATER_PULSE
	const EVENT_GOT_TM57_CHARGE_BEAM
	const EVENT_GOT_TM19_GIGA_DRAIN
	const EVENT_GOT_TM54_POISON_JAB
	const EVENT_GOT_TM04_CALM_MIND
	const EVENT_GOT_TM35_FLAMETHROWER
	const EVENT_GOT_TM26_EARTHQUAKE
; Johto story events           ;+5
...
; Crystal-exclusive events in Johto
	...
	const EVENT_BEAT_RIVAL_CHERRYGROVE
...
; Kanto story events
	...
	const EVENT_GOT_TM43_RAZOR_WIND
...
; Camper
	...
	const EVENT_BEAT_YOUNGSTER_LLOYD
...
; Johto itemballs
	...
	const EVENT_GOT_TM49_AURA_SPHERE
...
; Unused: next 48 events
	...
	const EVENT_MEWTWO_CAVE_MEWTWO
	const EVENT_SUNDAY_SUNFLORA
	const EVENT_SUNDAY_ELECTABUZZ
	const EVENT_MONDAY_SNORLAX
	const EVENT_MONDAY_MAGMAR
	const EVENT_TUESDAY_FARFETCHD
	const EVENT_TUESDAY_MAROWAK
	const EVENT_WEDNESDAY_SMEARGLE
	const EVENT_WEDNESDAY_AERODACTYL
	const EVENT_THURSDAY_SUDOWOODO
	const EVENT_THURSDAY_DRAGONAIR
	const EVENT_FRIDAY_RAPIDASH
	const EVENT_SATURDAY_LICKITUNG
	const EVENT_SATURDAY_EEVEE
	const EVENT_CRYSTAL_ONIX
	const EVENT_FOUGHT_ONIX
	const EVENT_MEW
	const EVENT_FOUGHT_MEW
	const EVENT_FOUGHT_ZAPDOSG
	const EVENT_RUIN_VALLEY_ZAPDOS
; Rematches
	const EVENT_FALKNER_REMATCH_ACTIVE
	const EVENT_BUGSY_REMATCH_ACTIVE
	const EVENT_WHITNEY_REMATCH_ACTIVE
	const EVENT_MORTY_REMATCH_ACTIVE
	const EVENT_JASMINE_REMATCH_ACTIVE
	const EVENT_CHUCK_REMATCH_ACTIVE
	const EVENT_PRYCE_REMATCH_ACTIVE
	const EVENT_CLAIR_REMATCH_ACTIVE
